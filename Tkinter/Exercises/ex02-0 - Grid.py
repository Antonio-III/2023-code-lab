...
Label(root,
      text = 'Find:').grid(sticky=E) # row/column is 0 by default. We count by cross axis
...
Button(root, text = 'Find').grid(row = 0, column =4,sticky='we')
...
Entry(root).grid(row=1,column=1, columnspan=3,sticky='we',padx=2,pady=2)
