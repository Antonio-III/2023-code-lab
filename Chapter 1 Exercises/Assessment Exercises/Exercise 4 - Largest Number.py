...
three_nums = {"first":"","second":"","third":""} # Initialize a list that will collect the 3 numbers entered
...
for n in three_nums.keys(): # This for loop iterates as much as the amount of items in the list 
    ...
    three_nums[n] = entered_num
    index = three_nums.index(max(sorted(three_nums, reverse = True))) # Store the index value of the highest number in the list. To be used later 
...
# In case of all numbers being equal or that there are 2 greatest numbers
    ...
    elif three_nums[0] == threenums[2]:
        print(f"The {number_place[index]} and {number_place[index+2]} numbers are both the greatest numbers, being {three_nums[index]}.")
    else:
