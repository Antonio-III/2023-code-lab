...
sorted_nums = dict(sorted(threenums.items(), key = lambda item: item[1], reverse = True)) #Sort the dictionary by their values-pair in descending order by using a custom sorting key. This returns a list-type so we use dict() to convert the list into a dict-type
